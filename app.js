...
function generateCsvPreview() {
  ...
  const cardId = document.getElementById('mapCreditCard')?.value?.trim();

  console.log('Valores capturados:', { dateCol, descCol, amountCol, cardId });

  // Validar se todos foram selecionados
  if (!dateCol || !descCol || !amountCol || !cardId) {
    console.error('Campos obrigatórios não preenchidos!');
    console.log('Valores:', { dateCol, descCol, amountCol, cardId });
    alert('⚠️ Preencha todos os campos obrigatórios:\n✓ Data\n✓ Descrição\n✓ Valor\n✓ Cartão de Crédito');
    return;
  }
  ...
  const card = creditCards.find(c => c.id === cardId);
  if (!card) {
    console.error('Cartão não encontrado! ID:', cardId);
    console.log('Cartões disponíveis:', creditCards.map(c => ({ id: c.id, name: c.holder_name })));
    alert('❌ Cartão não encontrado!\nCartão ID: ' + cardId);
    return;
  }
  ...
  console.log('csvMapping salvo:', csvMapping);
  ...
  console.log('✅ Transição para csvPreviewSection concluída');
}
