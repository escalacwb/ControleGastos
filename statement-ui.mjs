...
export function openStatementEditor({card,cycle,month,rows,showDialog,formWrap,rpc,client,pay=false,autoImport=false}) {
  ...
  showDialog(cycle?'Fatura · detalhes e pagamento':'Registrar fatura',formWrap(body,'Salvar fatura'),async form=>{
    ...
    if(resolvedCycle&&Math.abs(cents(resolvedCycle.total_spent)-cents(amount))>5)throw Error('Já existe uma fatura nesta referência com outro total. Abra Detalhar / anexar nessa fatura.');
  ...
  async function checkHistory(){
    ...
    try{
      ...
      const matches=existing.filter(t=>t.close_amount&&t.account_id);
