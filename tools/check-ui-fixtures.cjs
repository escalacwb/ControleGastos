...
db.billing_cycles.push({...db.billing_cycles[0],id:randomUUID(),total_spent:99999,total_paid:0,archived_at:new Date().toISOString()});
...
await page.locator('.nav-link[href="#cards"]').click();assert.equal(await page.locator('[data-action="pay-cycle"]').count(),1,'Archived invoice must not appear');await page.locator('[data-action="pay-cycle"]').first().click();await page.locator('#statement-history').filter({hasText:'Nenhum pagamento'}).waitFor();await page.locator('[name="confirm_new_payment"]').check();await page.locator('[name="pay_amount"]').fill('250,00');await page.locator('select[name="pay_account"]').selectOption(a1);await page.locator('#edit-form button[type="submit"]').click();await page.locator('#dialog').waitFor({state:'hidden'});assert.equal(calls.at(-1).name,'save_statement_bundle');assert.equal(calls.at(-1).body.p_data.pay_amount,250);
